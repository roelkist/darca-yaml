...
def test_load_yaml_file_read_error():
    with patch("darca_file_utils.file_utils.FileUtils.read_file", side_effect=IOError("boom")):
...
def test_save_yaml_file_with_validation(tmp_path, valid_yaml_data, valid_yaml_schema):
    file_path = tmp_path / "validated.yaml"
    result = YamlUtils.save_yaml_file(str(file_path), valid_yaml_data, validate=True, schema=valid_yaml_schema)
    assert result
    assert file_path.exists()
...
def test_save_yaml_file_validation_schema_missing(tmp_path, valid_yaml_data):
    with pytest.raises(YamlUtilsException) as exc:
        YamlUtils.save_yaml_file(str(tmp_path / "x.yaml"), valid_yaml_data, validate=True)
...
def test_save_yaml_file_validation_failure(tmp_path, valid_yaml_schema):
    ...
    with pytest.raises(YamlUtilsException) as exc:
        YamlUtils.save_yaml_file(str(tmp_path / "bad.yaml"), invalid_data, validate=True, schema=valid_yaml_schema)
    assert "YAML_VALIDATION_ERROR" in str(exc.value)


def test_save_yaml_file_serialize_failure(tmp_path):
    class NonSerializable:
        pass

    with pytest.raises(YamlUtilsException) as exc:
        YamlUtils.save_yaml_file(str(tmp_path / "fail.yaml"), {"obj": NonSerializable()})
    assert "YAML_SERIALIZE_ERROR" in str(exc.value)


def test_save_yaml_file_write_failure(tmp_path, valid_yaml_data):
    with patch("darca_file_utils.file_utils.FileUtils.write_file", side_effect=IOError("disk full")):
        with pytest.raises(YamlUtilsException) as exc:
            YamlUtils.save_yaml_file(str(tmp_path / "fail.yaml"), valid_yaml_data)
